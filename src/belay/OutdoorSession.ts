...
export class OutdoorSession {
  ...
  clip(quiet=false){if(!this.canClip)return false;const draw=this.nextDraw!;draw.clip();this.climber.playClip(draw.ropePoint());this.audio.play('clip');if(!quiet)this.notify(`Clipped. ${this.clipped} of ${this.draws.length} protection points.`);return true;}
  fall(forced=false){
    if(!this.climb.active&&!forced)return;
    if(this.clipped===0&&!this.topAnchor&&!forced){this.notify('Clip the first quickdraw before testing a catch.');return;}
    const height=this.climber.group.position.y+.85;
    this.climb.stop();this.suspended=true;this.reacted=false;
    this.belay.beginFall(height,this.topAnchor?.y??this.draws[this.clipped-1]?.position.y??0);this.notify('Falling — your partner has the rope.');
  }
  ...
  reset(){this.climb.stop();this.belay.reset();this.suspended=false;this.role='climber';this.reacted=false;for(const d of this.draws)d.reset();this.partner.group.position.copy(this.baseBelayer);}
  update(dt:number,time:number){
    ...
    const points=[device,...(this.topAnchor?[this.topAnchor]:this.draws.filter(d=>d.clipped).map(d=>d.ropePoint())),this.climber.harnessPosition()];
    this.rope.setView(this.role==='climber'&&(this.climb.active||this.suspended)?'climber':'full');
    this.rope.group.visible=!this.topAnchor||this.climb.active||this.suspended;this.ropePoints=points.length;this.rope.update(dt,points,this.belay.slack,this.belay.tension,time);
