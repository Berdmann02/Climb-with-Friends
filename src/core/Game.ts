...
export class Game {
  ...
  private readonly indoorRope=new RopeController();
  ...
  constructor(root:HTMLElement){
    ...
    this.scene.add(this.player.group,this.partner.group,this.holds.group,this.indoorRope.group);
  ...
  private async changeLocation(location:LocationId,menu=false){
    ...
    this.route=this.routes.get(location)??createDefaultRoute(location);this.routes.set(location,this.route);this.holds.setRoute(this.route);this.climb.setRoute(this.route);
    ...
    this.outdoor=new OutdoorSession(this.player,this.partner,this.climb,world.protection,world.belayPosition,this.audio,m=>this.ui.toast(m));
    if(location==='outdoor'){this.scene.add(this.outdoor.rope.group);for(const d of this.outdoor.draws)this.scene.add(d.group);}
    this.player.group.visible=!menu;this.partner.group.visible=!menu;this.indoorRope.group.visible=location==='gym'&&!menu;this.manualBelayAction='neutral';
  ...
  action(action:UIAction){
    ...
    switch(action){
      ...
      case 'clip':this.outdoor.clip();break;case 'fall':if(this.location==='outdoor')this.outdoor.fall();break;
      case 'role':if(this.location==='outdoor'){
        ...
        this.cameraRig.yaw=.05;this.cameraRig.pitch=.25;this.ui.toast(this.outdoor.role==='belayer'?'You have the rope. Hold F to feed; Space to brake.':'You’re on the wall. Your partner has the rope.');
      }break;
  ...
  private startClimb(){
    ...
    if(this.climb.start()){this.movement.reset();this.finishShown=false;this.audio.play('hold');this.cameraRig.yaw=.18;this.cameraRig.pitch=.15;this.ui.toast('Find your rhythm. W or Space reaches the next hold.');}
    else this.ui.toast('These start holds are too far apart. Bring them closer together.');
  }
  ...
  private pickHold(e:MouseEvent){if(!this.climb.active||this.editor.active||this.menu||this.outdoor.role==='belayer')return;const ray=new THREE.Raycaster();ray.setFromCamera(new THREE.Vector2(e.clientX/innerWidth*2-1,-e.clientY/innerHeight*2+1),this.camera);const hit=ray.intersectObject(this.holds.group,true).find(h=>h.object.userData.holdId);if(hit&&this.climb.reachHold(hit.object.userData.holdId))this.audio.play('hold');}
  private controls(){
    ...
    if(input.consume('KeyL')&&this.location==='outdoor')this.action('lower');
    if(this.outdoor.role==='belayer'){
      ...
      const dir=input.down('KeyS')?'down':input.down('KeyA')?'left':input.down('KeyD')?'right':input.down('KeyW')||input.down('Space')?'up':null;
      if(dir&&this.climb.move(dir))this.audio.play('hold');
    }
  }
    ...
    if(!this.loading&&this.world){
      ...
      if(!this.paused&&!modal){this.time+=dt;
        ...
        if(!this.menu&&!this.editor.active){
          if(this.climb.active){this.climb.update(dt,this.time);this.player.update(dt,this.time,'climb');}
          ...
          if(this.location==='outdoor')this.outdoor.update(dt,this.time);else{
            this.partner.setBelayAction(this.climb.active?'feed':'neutral',.1);this.partner.update(dt,this.time,'belay');
            this.indoorRope.group.visible=this.climb.active;
            if(this.climb.active){
              const device=this.partner.harnessPosition().add(new THREE.Vector3(0,.05,-.05));
              this.indoorRope.update(dt,[device,new THREE.Vector3(0,7.9,.35),this.player.harnessPosition()],.25,.35,this.time);
              this.indoorRope.updateBrake([device,this.partner.group.localToWorld(new THREE.Vector3(.15,.72,-.3)),new THREE.Vector3(1.4,.06,6.5)],this.time,.1);
            }
          }
          if(this.climb.finished&&!this.finishShown){this.finishShown=true;this.audio.play('save');this.ui.toast('Top! A little higher, together. Beautifully done.');}
        }
  ...
  private updateHUD(){
    ...
    else if(belaying)this.ui.context(this.outdoor.belay.slack<.2?'Your partner needs a little rope.':'You have the rope.','Hold F to feed · G to take · Space to brake · L to lower','Switch to climber','role');
    else if(this.climb.finished)this.ui.context('A little higher, together.','You reached the finish. Take in the view.',outdoor?'Lower to the ground':'Back to the lounge',outdoor?'lower':'reset');
    else if(this.climb.active&&outdoor&&this.outdoor.canClip)this.ui.context('A good place to clip.','C clips the rope · W keeps you moving','Clip quickdraw','clip');
    else if(this.climb.active)this.ui.context('Trust your feet. Find the next hold.','W / Space to reach · A S D for direction · Click a reachable hold','Back to the ground','reset');
